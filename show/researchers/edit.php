...
{   
    ...
    $sql2 = 'SELECT `note` FROM `user` WHERE `username` = ?';
    $query2 = $connect->prepare($sql2);
    $query2->bindValue(1,$patient_name);
    $query2->execute();
    if($query2->rowCount() == "")
    {
    ...
    {
        $value = $query2->fetch(PDO::FETCH_OBJ);
    }
...
<div class="text-secondary" style='text-align:center;margin:50px auto;width:400px;height:400px;background-color:#cccccc;border-radius:10px;border:2px solid blue;'>
    ...
    <input type='text' name='note' value="<?php echo $value->note;?>"style='width:80%;height:100px;text-align:center;border-radius:10px;margin-top:50px;font-size:20px;' placeholder=''>
